...
import {
  ...
  LogLevel,
  Config
} from 'traaittplatform-wallet-backend';
...
export default class WalletSession {
  ...
  getFiatPrice = async (fiat: string) => {
    const apiURL = `https://api.coingecko.com/api/v3/coins/markets?vs_currency=${fiat}&ids=traaittplatform&order=market_cap_desc&per_page=100&page=1&sparkline=false&price_change_percentage=7d`;
  ...
  atomicToHuman(x: number, prettyPrint?: boolean) {
    if (prettyPrint || false) {
      return `${this.formatLikeCurrency((x / 100000000).toFixed(8))}`;
    }
    return x / 100000000;
  }
