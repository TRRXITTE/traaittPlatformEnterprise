...
export default class Send extends Component<Props, State> {
  ...
  handleAmountChange = (event: any) => {
    ...
    const totalAmount = (
      ...
      fee +
      parseFloat(session.daemon.feeAmount / 100000000)
    ).toFixed(8);
    this.setState({
      enteredAmount,
      totalAmount
  ...
  handleTotalAmountChange = (event: any) => {
    ...
    const subtractFee =
      Number(totalAmount) * 100000000 - 100000000 - parseInt(session.daemon.feeAmount, 10);

    const enteredAmount =
      subtractFee < 0
        ? ''
        : session.atomicToHuman(subtractFee, false).toFixed(8);
  ...
  confirmTransaction = (event: any) => {
    ...
    const sufficientFunds =
      (session.getUnlockedBalance() + session.getLockedBalance()) / 100000000 >=
      Number(totalAmount);

    const sufficientUnlockedFunds =
      session.getUnlockedBalance() > Number(totalAmount) / 100000000;
  ...
  createTestTransaction = async () => {
    ...
    const amount = Math.floor(Math.random() * 100000000) + 1;
    const paymentID = this.generatePaymentID();

    await this.setState({
      selectedContact: { label: sendToAddress, value: sendToAddress },
      enteredAmount: String(amount / 100000000),
      totalAmount: String((amount + 10) / 100000000),
      sendToAddress,
      paymentID
    });
    ...
    const addressInput = (
      ...
      >
        <Creatable
          ...
          placeholder="Enter a traaittPlatform address or a contact name to send funds to"
