...
export default class Balance extends Component<Props, State> {
  ...
  switchCurrency = () => {
    ...
    if (displayCurrency === 'fiat') {
      ...
      session.modifyConfig('displayCurrency', 'ETRX');
      eventEmitter.emit('modifyCurrency', 'ETRX');
    }
    ReactTooltip.rebuild();
  };
  ...
  render() {
    ...
    if (session.wallet && displayCurrency === 'ETRX') {
      balanceTooltip =
        `Unlocked: ${session.atomicToHuman(unlockedBalance, true)} ${
          il8n.ETRX
        }<br>` +
        `Locked: ${session.atomicToHuman(lockedBalance, true)} ${il8n.ETRX}`;
    } else if (
      session.wallet &&
