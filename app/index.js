...
import { WalletBackend } from 'traaittplatform-wallet-backend';
...
ipcRenderer.on('handleSaveAs', () => {
  ...
  const options = {
    ...
    filters: [
      {
        name: 'traaittplatform Wallet File (v0)',
        extensions: ['wallet']
      }
    ]
  };
...
eventEmitter.on('sendNotification', function sendNotification(amount) {
  ...
  if (notifications) {
    const notif = new window.Notification('Transaction Received!', {
      body: `${il8n.just_received} ${amount} ${il8n.ETRX}`
    });
    if (notif) {
      log.debug(`Sent notification: You've just received ${amount} ETRX.`);
    }
  }
});
